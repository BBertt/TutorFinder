                    ...
                    <div className="flex flex-col lg:flex-row shadow-lg my-10">
                        <div className="w-full lg:w-3/4 bg-white p-10">
                            <div className="flex justify-between border-b pb-8">
                                <h1 className="font-semibold text-2xl">{cartItems.length} Total in Cart</h1>
                            </div>
                                    ...
                                    >
                                        ...
                                        <div className="flex-grow ml-4">
                                            ...
                                            <p className="text-sm text-gray-600">
                                                {item.course.user ? item.course.user.name : 'Author'}
                                            </p>
                                            <p className="font-bold text-gray-800 mt-1">
                                                Rp {parseFloat(item.course.price).toLocaleString('id-ID')}
                                            </p>
                                        </div>
                                        <div className="ml-4">
                        ...
                        <div className="w-full lg:w-1/4 bg-[#4F6D40] text-white p-8 rounded-r-lg">
                            <h1 className="font-bold text-2xl border-b pb-6">Total</h1>
                            <div className="mt-8 space-y-4">
                                {cartItems.map((item) => (
                                    <div key={item.id} className="flex justify-between">
                                        <span className="w-1/2 truncate">{item.course.title}</span>
                                        <span>Rp {parseFloat(item.course.price).toLocaleString('id-ID')}</span>
                                    </div>
                                ))}
                            </div>
